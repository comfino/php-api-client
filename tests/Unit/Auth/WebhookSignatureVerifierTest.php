...
final class WebhookSignatureVerifierTest extends TestCase
{
    ...
    {
        /* With an empty key the expected signature is a hash of the payload alone, which the caller can compute
           without knowing any secret. Verifying it would authorize every forged request. */
        $body = '{"status":"ACCEPTED","orderId":"12345"}';

        $this->assertFalse($this->verifier->verify(hash('sha3-256', $body), '', $body));
    }

    public function testRejectsEmptyApiKeyForAnEmptyPayload(): void
    {
        $this->assertFalse($this->verifier->verify(hash('sha3-256', ''), '', ''));
    }
}
