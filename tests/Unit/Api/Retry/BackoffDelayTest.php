...
final class BackoffDelayTest extends TestCase
{
    ...
    public function testExecutorSleepsBetweenAttempts(): void
    {
        ...
        $executor = new RetryExecutor(
            new ExponentialBackoffRetryPolicy(new TimeoutConfig(1, 3), 3, null, 100, 2000),
            $sleeper
        );
    ...
    public function testRetryAfterOverridesTheBackoffDelay(): void
    {
        ...
        $sleeper = new NoDelaySleeper();
        $executor = new RetryExecutor(
            new ExponentialBackoffRetryPolicy(new TimeoutConfig(1, 3), 2, null, 100, 2000),
            $sleeper
        );
        ...
        try {
            $executor->execute(
                static fn () => throw new \Comfino\Api\Retry\RetryableResponse($response)
            );
        } catch (Throwable) {
            // Expected once the attempts are spent.
        }
    ...
    public function testNonIdempotentRequestIsNotReplayed(): void
    {
        $calls = 0;
        $executor = new RetryExecutor(
            new ExponentialBackoffRetryPolicy(new TimeoutConfig(1, 3), 3),
            new NoDelaySleeper()
        );
    ...
    public function testCallbackTimeoutAwareClientProducesANewInstancePerTimeoutConfig(): void
    {
        ...
        $this->assertSame(
            ['TimeoutConfig(connection=1s, transfer=3s)', 'TimeoutConfig(connection=2s, transfer=6s)'],
            $built
        );
    }
