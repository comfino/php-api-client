...
final class ErrorClassifierTest extends TestCase
{
    ...
    public function testPlainRuntimeExceptionIsFatal(): void
    {
        $this->assertSame(
            RetryVerdict::Fatal,
            (new ErrorClassifier())->classify(new RuntimeException('boom'))->verdict
        );
    }
    ...
    {
        $this->assertTrue(
            (new ErrorClassifier())->classifyResponse($this->psr17Factory->createResponse($statusCode))->isRetryable()
        );
    }
    ...
    {
        $this->assertFalse(
            (new ErrorClassifier())->classifyResponse($this->psr17Factory->createResponse(404))->isRetryable()
        );
    }
    ...
    public function testSuccessfulResponseClassifiesAsSuccess(): void
    {
        $this->assertSame(
            ...
            (new ErrorClassifier())->classifyResponse($this->psr17Factory->createResponse(200))->verdict
        );
    }
    ...
    {
        $classification = (new ErrorClassifier())->classifyResponse(
            $this->psr17Factory->createResponse(429)->withHeader('Retry-After', '2')
        );
    ...
    public function testRetryAfterHttpDateIsHonored(): void
    {
        ...
        $classification = $classifier->classifyResponse(
            $this->psr17Factory->createResponse(503)->withHeader('Retry-After', $httpDate)
        );
    ...
    public function testRetryAfterIsClampedToTheCeiling(): void
    {
        ...
        $classification = (new ErrorClassifier(5000))->classifyResponse(
            $this->psr17Factory->createResponse(429)->withHeader('Retry-After', '3600')
        );
    ...
    {
        $classification = (new ErrorClassifier())->classifyResponse(
            $this->psr17Factory->createResponse(429)->withHeader('Retry-After', 'soon-ish')
        );
