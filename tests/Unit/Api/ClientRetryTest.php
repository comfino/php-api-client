...
final class ClientRetryTest extends TestCase
{
    ...
    private function makeAlwaysRetryPolicy(
        ...
        $policy->method('classify')->willReturn(Classification::retry());
        $policy->method('isRetryable')->willReturn(true);
    ...
    public function testUpdateTimeoutsIsCalledOnRetryWhenClientIsTimeoutAware(): void
    {
        ...
        /* The timeouts are now derived per attempt, before the attempt, rather than pushed onto the transport from a
           retry callback afterwards - so both attempts are configured, each with its own values. */
        $this->assertCount(2, $timeoutAwareClient->updateTimeoutCalls);

        $this->assertSame([$policy->getConnectionTimeout(1), $policy->getTransferTimeout(1)], $timeoutAwareClient->updateTimeoutCalls[0]);
        $this->assertSame([$policy->getConnectionTimeout(2), $policy->getTransferTimeout(2)], $timeoutAwareClient->updateTimeoutCalls[1]);
    }
    ...
    public function testUpdateTimeoutsIsNotCalledForNonTimeoutAwareClient(): void
    {
        ...
        $result = $this->makeClient(new RetryExecutor(new ExponentialBackoffRetryPolicy(new TimeoutConfig(1, 3), 2)))->isShopAccountActive();
