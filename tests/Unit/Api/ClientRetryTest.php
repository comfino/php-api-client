...
final class ClientRetryTest extends TestCase
{
    ...
    private function makeAlwaysRetryPolicy(
        ...
        $policy->method('hasAttemptsLeft')->willReturnCallback(
            static fn (int $attempt): bool => $attempt < $maxAttempts
        );
    ...
    public function testUpdateTimeoutsIsCalledOnRetryWhenClientIsTimeoutAware(): void
    {
        ...
        $this->assertSame(
            [$policy->getConnectionTimeout(1), $policy->getTransferTimeout(1)],
            $timeoutAwareClient->updateTimeoutCalls[0]
        );
        $this->assertSame(
            [$policy->getConnectionTimeout(2), $policy->getTransferTimeout(2)],
            $timeoutAwareClient->updateTimeoutCalls[1]
        );
    }
