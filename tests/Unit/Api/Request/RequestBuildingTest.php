...
final class RequestBuildingTest extends TestCase
{
    ...
    public function testGetFinancialProductsWithMaxTermConstraint(): void
    {
        ...
        $psrRequest = $this->build(
            new GetFinancialProducts(new LoanQueryCriteria(loanAmount: 150000, allowedProductsConfig: $config))
        );
    ...
    public function testGetFinancialProductsWithTermsWhitelist(): void
    {
        ...
        $psrRequest = $this->build(
            new GetFinancialProducts(new LoanQueryCriteria(loanAmount: 50000, allowedProductsConfig: $config))
        );
    ...
    public function testCreateOrderBodyContainsAllowedProductsConfig(): void
    {
        ...
        $psrRequest = $this->build(
            new CreateOrder($this->createMockOrder(allowedProductsConfig: $config), 'api-key', false)
        );
        $body = $this->decodeBody($psrRequest);
    ...
    public function testReportShopPluginErrorSetsMessageVersionHeader(): void
    {
        ...
        $this->assertSame(
            (string) ReportShopPluginError::MESSAGE_VERSION,
            $psrRequest->getHeaderLine('Comfino-Message-Version')
        );
    }
    ...
    public function testExtractApiEndpointStripsHostAndQuery(): void
    {
        $this->assertSame(
            ...
            ReportShopPluginError::extractApiEndpoint('https://api.comfino.pl/v1/financial-products?foo=bar')
        );
        $this->assertSame(
            'v1/log-plugin-error',
            ReportShopPluginError::extractApiEndpoint('https://api.comfino.pl/v1/log-plugin-error')
        );
    ...
    public function testReportShopPluginErrorStripsAbsolutePathsFromErrorMessage(): void
    {
        $error = $this->makeShopPluginError(
            errorCode: 'E_PARSE',
            errorMessage: 'Error E_PARSE in /var/www/html/shop/comfino/src/Foo.php:42'
        );
        $psrRequest = $this->build(new ReportShopPluginError($error, self::HASH_KEY));
        $details = $this->decodeReportBody($psrRequest);
