...
final class RequestBuildingTest extends TestCase
{
    ...
    /**
     * Builds a ShopPluginError with sensible defaults, overriding only the fields a test cares about.
     *
     * @param array<array-key, mixed> $environment
     */
    private function makeShopPluginError(
     ...
     */
    private function createMockOrder(string $orderId = 'order-1', ?array $allowedProductsConfig = null): OrderInterface
    {
    ...
    public function testGetWidgetTypesUsesGetMethodAndCorrectPath(): void
    {
        ...
        $this->assertSame('GET', $psrRequest->getMethod());
        $this->assertStringContainsString('/widget/v1/widget-types', (string) $psrRequest->getUri());
        $this->assertEmpty((string) $psrRequest->getBody());
    }
    ...
    {
        $psrRequest = $this->build(new IsShopAccountActive('https://shop.test/cache', 'https://shop.test/config'));
    ...
    public function testGetFinancialProductsWithMaxTermConstraint(): void
    {
        $config = [
            new AllowedProductConfig(LoanType::INSTALLMENTS_ZERO_PERCENT, maxTerm: 12),
            new AllowedProductConfig(LoanType::CONVENIENT_INSTALLMENTS, maxTerm: 24),
        ];

        $psrRequest = $this->build(new GetFinancialProducts(new LoanQueryCriteria(loanAmount: 150000, allowedProductsConfig: $config)));

        $uri = urldecode((string) $psrRequest->getUri());

        $this->assertStringContainsString('allowedProductsConfig[0][type]=INSTALLMENTS_ZERO_PERCENT', $uri);
        $this->assertStringContainsString('allowedProductsConfig[0][maxTerm]=12', $uri);
        $this->assertStringContainsString('allowedProductsConfig[1][type]=CONVENIENT_INSTALLMENTS', $uri);
        $this->assertStringContainsString('allowedProductsConfig[1][maxTerm]=24', $uri);
        $this->assertStringNotContainsString('minTerm', $uri);
        $this->assertStringNotContainsString('terms', $uri);
    }

    public function testGetFinancialProductsWithTermsWhitelist(): void
    {
        $config = [new AllowedProductConfig(LoanType::PAY_LATER, terms: [1, 3, 6])];

        $psrRequest = $this->build(new GetFinancialProducts(new LoanQueryCriteria(loanAmount: 50000, allowedProductsConfig: $config)));

        $uri = urldecode((string) $psrRequest->getUri());

        $this->assertStringContainsString('allowedProductsConfig[0][type]=PAY_LATER', $uri);
        $this->assertStringContainsString('allowedProductsConfig[0][terms][0]=1', $uri);
        $this->assertStringContainsString('allowedProductsConfig[0][terms][1]=3', $uri);
        $this->assertStringContainsString('allowedProductsConfig[0][terms][2]=6', $uri);
        $this->assertStringNotContainsString('maxTerm', $uri);
    }

    public function testGetFinancialProductsWithoutAllowedProductsConfigOmitsParam(): void
    {
        $psrRequest = $this->build(new GetFinancialProducts(new LoanQueryCriteria(loanAmount: 150000)));

        $this->assertStringNotContainsString('allowedProductsConfig', (string) $psrRequest->getUri());
    }
    ...
    public function testCreateOrderSignatureChangesWithApiKey(): void
    {
        ...
        $this->assertNotSame($psr1->getHeaderLine('Comfino-Order-Signature'), $psr2->getHeaderLine('Comfino-Order-Signature'));
    }
    ...
    public function testCreateOrderHashesAreSha3256HexStrings(): void
    {
        ...
        $this->assertMatchesRegularExpression('/^[0-9a-f]{64}$/', $psrRequest->getHeaderLine('Comfino-Order-Signature'));
    }

    /**
     * @throws JsonException
     */
    public function testCreateOrderBodyContainsAllowedProductsConfig(): void
    {
        $config = [
            ...
            new AllowedProductConfig(LoanType::CONVENIENT_INSTALLMENTS, terms: [6, 12, 24]),
        ];

        $psrRequest = $this->build(new CreateOrder($this->createMockOrder(allowedProductsConfig: $config), 'api-key', false));
        $body = $this->decodeBody($psrRequest);

        $this->assertArrayHasKey('allowedProductsConfig', $body);
        $this->assertCount(3, $body['allowedProductsConfig']);
        $this->assertSame('PAY_LATER', $body['allowedProductsConfig'][0]['type']);
        $this->assertSame(6, $body['allowedProductsConfig'][0]['maxTerm']);
        $this->assertArrayNotHasKey('minTerm', $body['allowedProductsConfig'][0]);
        $this->assertArrayNotHasKey('terms', $body['allowedProductsConfig'][0]);
        $this->assertSame(3, $body['allowedProductsConfig'][1]['minTerm']);
        $this->assertSame(24, $body['allowedProductsConfig'][1]['maxTerm']);
        $this->assertSame([6, 12, 24], $body['allowedProductsConfig'][2]['terms']);
        $this->assertArrayNotHasKey('maxTerm', $body['allowedProductsConfig'][2]);
    }

    /**
     * @throws JsonException
     */
    public function testCreateOrderWithoutNewParamsBodyIsUnchanged(): void
    {
        $psrRequest = $this->build(new CreateOrder($this->createMockOrder(), 'api-key'));
        $body = $this->decodeBody($psrRequest);

        $this->assertArrayNotHasKey('allowedProductsConfig', $body);
        $this->assertArrayHasKey('loanParameters', $body);
    }
    ...
    {
        $error = $this->makeShopPluginError();
    ...
    public function testReportShopPluginErrorSetsMessageVersionHeader(): void
    {
        $error = $this->makeShopPluginError();
        $psrRequest = $this->build(new ReportShopPluginError($error, self::HASH_KEY));

        $this->assertSame((string) ReportShopPluginError::MESSAGE_VERSION, $psrRequest->getHeaderLine('Comfino-Message-Version'));
    }

    /**
     * @throws JsonException
     */
    public function testReportShopPluginErrorBodyContainsStructuredFields(): void
    {
        $error = $this->makeShopPluginError(
            errorCode: 'TypeError',
            errorMessage: 'Boom',
            apiEndpoint: 'v1/financial-products',
            occurredAt: 1729683272,
            category: ErrorCategory::ExceptionTypeError,
        ...
        $this->assertSame('1.2.3', $details['plugin_version']);
        $this->assertSame('8.1', $details['platform_version']);
        $this->assertSame('8.1.27', $details['php_version']);
        $this->assertSame('exception_type_error', $details['category']);
        $this->assertSame('critical', $details['severity']);
        $this->assertSame('api_communication', $details['context']);
        $this->assertSame('v1/financial-products', $details['api_endpoint']);
        $this->assertSame(1729683272, $details['occurred_at']);
    }

    public function testExtractApiEndpointStripsHostAndQuery(): void
    {
        $this->assertSame('v1/financial-products', ReportShopPluginError::extractApiEndpoint('https://api.comfino.pl/v1/financial-products?foo=bar'));
        $this->assertSame('v1/log-plugin-error', ReportShopPluginError::extractApiEndpoint('https://api.comfino.pl/v1/log-plugin-error'));
        $this->assertNull(ReportShopPluginError::extractApiEndpoint(null));
        $this->assertNull(ReportShopPluginError::extractApiEndpoint(''));
        $this->assertNull(ReportShopPluginError::extractApiEndpoint('https://api.comfino.pl'));
    }
    ...
    {
        $error = $this->makeShopPluginError(errorCode: 'E_PARSE', errorMessage: 'Error E_PARSE in /var/www/html/shop/comfino/src/Foo.php:42');
        $psrRequest = $this->build(new ReportShopPluginError($error, self::HASH_KEY));
        $details = $this->decodeReportBody($psrRequest);
    ...
    {
        $error = $this->makeShopPluginError(errorMessage: 'err');
        $psrRequest = $this->build(new ReportShopPluginError($error, self::HASH_KEY));
