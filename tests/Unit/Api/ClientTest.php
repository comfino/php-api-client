...
use Comfino\Tests\Unit\Api\Stub\CustomNoteResponse;
...
final class ClientTest extends TestCase
{
    ...
    private function createMockOrder(
        ...
        $order->method('getAllowedProductsConfig')->willReturn($allowedProductsConfig);
    ...
    public function testIsShopAccountActiveWithOptionalHeaders(): void
    {
        ...
        $this->client->isShopAccountActive('https://example-shop.test/cache-invalidate', 'https://example-shop.test/config');

        $request = $this->getLastRequest();

        $this->assertSame('https://example-shop.test/cache-invalidate', $request->getHeaderLine('Comfino-Cache-Invalidate-Url'));
    ...
    public function testGetProductTypesReturnsKnownEnums(): void
    {
        $responseBody = json_encode(
            [
                'INSTALLMENTS_ZERO_PERCENT' => ['Raty zero procent', 'Raty 0%'],
                'CONVENIENT_INSTALLMENTS' => ['Niskie raty', 'Niskie raty'],
                'PAY_LATER' => ['Zapłać później', 'Kup teraz, zapłać później'],
            ],
            JSON_THROW_ON_ERROR
        );
        ...
        $this->assertSame('GET', $request->getMethod());
        $this->assertStringContainsString('/v2/product-types', (string) $request->getUri());
    ...
    {
        $responseBody = json_encode(['PAY_LATER' => ['Zapłać później', 'Kup teraz, zapłać później']], JSON_THROW_ON_ERROR);
        ...
        $this->assertSame('Zapłać później', $response->productTypesWithNames['PAY_LATER']);
        $this->assertArrayHasKey('PAY_LATER', $response->productTypesWithPublicNames);
        $this->assertSame('Kup teraz, zapłać później', $response->productTypesWithPublicNames['PAY_LATER']);
    }
     ...
     * @throws JsonException
     */
    public function testGetUserSettingsReturnsFlagsKeyedByName(): void
    {
        $responseBody = json_encode(['flags' => ['FLAG_ONE' => [], 'FLAG_TWO' => ['maxAmount' => 50000]]], JSON_THROW_ON_ERROR);

        $this->mockHttpClient->addResponse($this->createJsonResponse(200, $responseBody));

        $response = $this->client->getUserSettings();
        ...
        $this->assertSame('GET', $request->getMethod());
        $this->assertStringContainsString('/v1/user/settings/flags', (string) $request->getUri());
    }

    // -------------------------------------------------------------------------
    // fetchAgreements / createUser
    // -------------------------------------------------------------------------
     ...
     * @throws JsonException
     */
    public function testFetchAgreementsReturnsParsedAgreements(): void
    {
        $responseBody = json_encode(
            [
                ['id' => 'terms', 'content' => 'I accept the terms.', 'required' => true],
                ['id' => 'newsletter', 'content' => 'Send me the newsletter.'],
            ],
            JSON_THROW_ON_ERROR
        );
        ...
        $response = $this->client->fetchAgreements();

        $this->assertCount(2, $response->agreements);
        $this->assertSame('terms', $response->agreements[0]->id);
        $this->assertTrue($response->agreements[0]->required);
        $this->assertSame('newsletter', $response->agreements[1]->id);
        $this->assertFalse($response->agreements[1]->required);

        $request = $this->getLastRequest();

        $this->assertSame('GET', $request->getMethod());
        $this->assertStringContainsString('/v1/fetch-agreements', (string) $request->getUri());
    }
    ...
    public function testCreateUserReturnsCredentials(): void
    {
        ...
        $responseBody = json_encode(['apiKey' => 'new-api-key', 'widgetKey' => 'new-widget-key'], JSON_THROW_ON_ERROR);
        ...
        $response = $clientWithoutKey->createUser($registration);

        $this->assertSame('new-api-key', $response->apiKey);
        $this->assertSame('new-widget-key', $response->widgetKey);

        $request = $this->getLastRequest();

        $this->assertSame('POST', $request->getMethod());
        $this->assertStringContainsString('/v1/user', (string) $request->getUri());
        $this->assertEmpty($request->getHeaderLine('Api-Key'));

        $sentBody = json_decode((string) $request->getBody(), true, 512, JSON_THROW_ON_ERROR);

        $this->assertSame('shop.example.com', $sentBody['name']);
        $this->assertSame(['terms'], $sentBody['agreements']);
    }

    /**
     * @throws ClientExceptionInterface
     */
    public function testCreateUserAlreadyRegisteredThrowsConflict(): void
    {
        $clientWithoutKey = new Client($this->mockHttpClient, $this->psr17Factory, $this->psr17Factory, null);
        ...
        $this->mockHttpClient->addResponse(
            $this->createJsonResponse(
                409,
                '{"errors":{"webSiteUrl":"is already registered"}}',
                'Conflict'
            )
        );
        ...
        $clientWithoutKey->createUser(new UserRegistration(
            'shop.example.com',
            'https://shop.example.com',
            'Jane Doe',
            'jane@example.com',
    ...
    public function testGetFinancialProductsWithLoanTypeSendsQueryParam(): void
    {
        ...
        $this->client->getFinancialProducts(new LoanQueryCriteria(loanAmount: 100000, loanType: LoanType::PAY_LATER));
    ...
    {
        $this->mockHttpClient->addResponse($this->psr17Factory->createResponse(200)->withHeader('Content-Type', 'application/json'));
    ...
    {
        $this->mockHttpClient->addResponse($this->createJsonResponse(401, '{"message":"Unauthorized"}', 'Unauthorized'));
    ...
    {
        $this->mockHttpClient->addResponse($this->createJsonResponse(403, '{"message":"Forbidden"}', 'Forbidden'));
    ...
    {
        $this->mockHttpClient->addResponse($this->createJsonResponse(400, '{"errors":{"orderId":"Order ID is required"}}', 'Bad Request'));
    ...
    {
        $this->mockHttpClient->addResponse($this->createJsonResponse(500, '{"message":"Internal Server Error"}', 'Internal Server Error'));
    ...
    public function testProductionModeUsesProductionBaseUrl(): void
    {
        ...
        $this->assertStringStartsWith(AbstractClient::PRODUCTION_API_BASE_URL, (string) $this->getLastRequest()->getUri());
    }
    ...
    public function testGetWidgetTypesReturnsParsedTypes(): void
    {
        ...
        $this->assertSame('GET', $request->getMethod());
        $this->assertStringContainsString('/widget/v1/widget-types', (string) $request->getUri());
    }
    ...
    public function testGetFinancialProductDetailsReturnsParsedProducts(): void
    {
        ...
        $response = $this->client->getFinancialProductDetails(new LoanQueryCriteria(loanAmount: 100000), $this->createMockCart());
    ...
    {
        $this->mockHttpClient->addResponse($this->createJsonResponse(400, '{"errors":{"orderId":"is required"}}', 'Bad Request'));
     ...
     * @throws JsonException
     */
    public function testValidateOrderWithAllowedProductsConfig(): void
    {
        $config = [
            new AllowedProductConfig(LoanType::INSTALLMENTS_ZERO_PERCENT, minTerm: 3, maxTerm: 24),
            new AllowedProductConfig(LoanType::PAY_LATER, maxTerm: 6),
        ];

        $this->mockHttpClient->addResponse($this->createJsonResponse(200, '{}'));

        $response = $this->client->validateOrder($this->createMockOrder(allowedProductsConfig: $config));
        ...
        $this->assertArrayHasKey('allowedProductsConfig', $body);
        $this->assertCount(2, $body['allowedProductsConfig']);
        $this->assertSame('INSTALLMENTS_ZERO_PERCENT', $body['allowedProductsConfig'][0]['type']);
        $this->assertSame(3, $body['allowedProductsConfig'][0]['minTerm']);
        $this->assertSame(24, $body['allowedProductsConfig'][0]['maxTerm']);
        $this->assertSame('PAY_LATER', $body['allowedProductsConfig'][1]['type']);
        $this->assertSame(6, $body['allowedProductsConfig'][1]['maxTerm']);
    }
     ...
     * @throws ClientExceptionInterface
     */
    public function testSendLoggedErrorSucceedsWithoutException(): void
    {
        $this->mockHttpClient->addResponse($this->psr17Factory->createResponse(200)->withHeader('Content-Type', 'application/json'));

        $this->client->sendLoggedError(
            new ShopPluginError(
                'shop.test',
                'WooCommerce',
                '1.2.3',
        ...
        $this->assertTrue(true, 'sendLoggedError completed without throwing.');
    }
     ...
     * @throws ClientExceptionInterface
     */
    public function testSendLoggedErrorThrowsOnNetworkFailure(): void
    {
    ...
    public function testSetClientHostnameIsReflectedInTrackIdHeader(): void
    {
        ...
        $this->assertStringStartsWith('my-shop.example.com-', $this->getLastRequest()->getHeaderLine('Comfino-Track-Id'));
    }
    ...
    {
        $this->client->setTrackId('checkout-cookie-track-id');

        $this->assertSame('checkout-cookie-track-id', $this->client->getTrackId());
    }

    public function testSetTrackIdRejectsInvalidValue(): void
    {
        $this->client->setTrackId("evil\r\nSet-Cookie: pwned=1");

        $this->assertNotSame("evil\r\nSet-Cookie: pwned=1", $this->client->getTrackId());

        $this->client->setTrackId(str_repeat('a', 129));

        $this->assertNotSame(str_repeat('a', 129), $this->client->getTrackId());
    }
    ...
    public function testSetTrackIdIsNoOpOnceMinted(): void
    {
        ...
        $mintedTrackId = $this->getLastRequest()->getHeaderLine('Comfino-Track-Id');

        $this->client->setTrackId('should-not-overwrite');

        $this->assertSame($mintedTrackId, $this->client->getTrackId());
    }
    ...
    public function testSetTrackIdIsReflectedInTrackIdHeader(): void
    {
        ...
        $this->assertSame('checkout-cookie-track-id', $this->getLastRequest()->getHeaderLine('Comfino-Track-Id'));
    }
    ...
    // -------------------------------------------------------------------------
    // setCustomApiBaseUrl - allowed destinations
    // -------------------------------------------------------------------------

    /** @dataProvider allowedApiBaseUrlProvider */
    public function testSetCustomApiBaseUrlAcceptsAllowedUrl(string $url): void
    {
        $this->client->setCustomApiBaseUrl($url);

        $this->assertSame($url, $this->client->getApiBaseUrl());
    }

    public function testSetCustomApiBaseUrlAcceptsNull(): void
    {
        $this->client->setCustomApiBaseUrl('https://api.comfino.pl');
        $this->client->setCustomApiBaseUrl(null);

        // After clearing, the sandbox URL is used because sandbox mode is on.
        $this->assertSame(AbstractClient::SANDBOX_API_BASE_URL, $this->client->getApiBaseUrl());
    }

    /** @return array<string, array{string}> */
    public static function allowedApiBaseUrlProvider(): array
    {
        return [
            'production domain' => ['https://api-ecommerce.comfino.pl'],
            'comfino.pl exact' => ['https://comfino.pl'],
            'comfino.pl subdomain' => ['https://staging.comfino.pl'],
            'craty.pl subdomain' => ['https://api-ecommerce.craty.pl'],
            'koszulawcraty.pl subdomain' => ['https://test.koszulawcraty.pl'],
            'loopback IPv4' => ['http://127.0.0.1'],
            'RFC 1918 class A' => ['http://10.0.0.5'],
            'RFC 1918 class B (Docker)' => ['http://172.17.0.1'],
            'RFC 1918 class C' => ['http://192.168.1.100'],
            'localhost' => ['http://localhost'],
            'localhost HTTPS' => ['https://localhost'],
            'Docker service name' => ['http://comfino-api'],
            'single-label HTTPS' => ['https://comfino-api'],
            'IPv6 loopback' => ['http://[::1]'],
            'private IP with port' => ['http://10.0.0.5:8080'],
            'Docker service with path' => ['http://comfino-api/v1'],
        ];
    }

    // -------------------------------------------------------------------------
    // setCustomApiBaseUrl - rejected destinations
    // -------------------------------------------------------------------------

    /** @dataProvider rejectedApiBaseUrlProvider */
    public function testSetCustomApiBaseUrlRejectsDisallowedUrl(string $url): void
    {
        $this->expectException(InvalidArgumentException::class);
    ...
    /** @return array<string, array{string}> */
    public static function rejectedApiBaseUrlProvider(): array
    {
        return [
            'arbitrary external domain' => ['https://evil.com'],
            'look-alike domain' => ['https://api.comfino.pl.evil.com'],
            'dotted internal not in allowlist' => ['https://my-mock-api.internal'],
            'comfino.pl over HTTP' => ['http://comfino.pl'],
            'subdomain of comfino.pl over HTTP' => ['http://api.comfino.pl'],
            'ftp scheme' => ['ftp://comfino.pl'],
            'malformed URL' => ['not-a-url'],
            'public IP' => ['https://8.8.8.8'],
            'empty string' => [''],
        ];
    }
     ...
     * @throws JsonException
     */
    public function testGetSupportedPlatformsReturnsParsedPlatforms(): void
    {
        $responseBody = json_encode(
            [
                'platforms' => [
                    ['code' => 'prestashop', 'name' => 'PrestaShop'],
        ...
        $response = $this->client->getSupportedPlatforms();

        $this->assertCount(2, $response->platforms);
        $this->assertSame('prestashop', $response->platforms[0]['code']);
        $this->assertSame('WooCommerce', $response->platforms[1]['name']);

        $request = $this->getLastRequest();

        $this->assertSame('GET', $request->getMethod());
        $this->assertStringContainsString('/v1/plugin-platforms', (string) $request->getUri());
        $this->assertSame(self::API_KEY, $request->getHeaderLine('Api-Key'));
    }
     ...
     * @throws JsonException
     */
    public function testGetLatestPluginReleaseReturnsParsedRelease(): void
    {
        $responseBody = json_encode(
            [
                'platform' => 'prestashop',
                'version' => '5.0.0',
                'tag_name' => 'v5.0.0',
                'download_url' => 'https://github.com/comfino/prestashop/releases/download/5.0.0/comfino.zip',
                'release_url' => 'https://github.com/comfino/prestashop/releases/tag/5.0.0',
                'prerelease' => false,
                'published_at' => '2026-07-02T12:00:00+00:00',
                'min_platform_version' => '1.7.7',
                'min_php_version' => '8.1',
                'description_html' => '<p>What\'s new</p>',
            ],
            JSON_THROW_ON_ERROR
        );
        ...
        $response = $this->client->getLatestPluginRelease('prestashop');

        $this->assertSame('prestashop', $response->platform);
        $this->assertSame('5.0.0', $response->version);
        $this->assertSame('1.7.7', $response->minPlatformVersion);
        $this->assertSame('8.1', $response->minPhpVersion);
        $this->assertFalse($response->prerelease);
        $this->assertSame('https://github.com/comfino/prestashop/releases/download/5.0.0/comfino.zip', $response->downloadUrl);

        $request = $this->getLastRequest();

        $this->assertSame('GET', $request->getMethod());
        $this->assertStringContainsString('/v1/plugin-releases/prestashop/latest', (string) $request->getUri());
    }
     ...
     * @throws JsonException
     */
    public function testGetLatestPluginReleaseDefaultsOptionalFieldsToNull(): void
    {
        $responseBody = json_encode(
            [
                'platform' => 'woocommerce',
                'version' => '5.0.0',
                'release_url' => 'https://github.com/comfino/woocommerce/releases/tag/5.0.0',
            ],
            JSON_THROW_ON_ERROR
        );
        ...
        $response = $this->client->getLatestPluginRelease('woocommerce');

        $this->assertSame('woocommerce', $response->platform);
        $this->assertNull($response->downloadUrl);
        $this->assertNull($response->minPlatformVersion);
        $this->assertNull($response->minPhpVersion);
        $this->assertNull($response->descriptionHtml);
        $this->assertFalse($response->prerelease);
    }
     ...
     * @throws JsonException
     */
    public function testSendCustomRequestReturnsFreeFormBodyByDefault(): void
    {
        $responseBody = json_encode(['note' => 'hello', 'pinned' => true], JSON_THROW_ON_ERROR);

        $this->mockHttpClient->addResponse($this->createJsonResponse(200, $responseBody));

        $response = $this->client->sendCustomRequest(new CustomRequest('POST', 'orders/order-123/notes', ['note' => 'hello']));

        $this->assertInstanceOf(CustomResponse::class, $response);
        $this->assertSame(['note' => 'hello', 'pinned' => true], $response->body);

        $request = $this->getLastRequest();

        $this->assertSame('POST', $request->getMethod());
        $this->assertStringContainsString('/v1/orders/order-123/notes', (string) $request->getUri());
        $this->assertSame('{"note":"hello"}', (string) $request->getBody());
    }
     ...
     * @throws JsonException
     */
    public function testSendCustomRequestSupportsQueryParamsAndCustomHeaders(): void
    {
        $this->mockHttpClient->addResponse($this->createJsonResponse(200, 'null'));

        $this->client->sendCustomRequest(
            new CustomRequest(
                'GET',
                'orders/order-123/notes',
                null,
     ...
     * @throws JsonException
     */
    public function testSendCustomRequestSupportsATypedResponseClass(): void
    {
        $responseBody = json_encode(['note' => 'hello'], JSON_THROW_ON_ERROR);

        $this->mockHttpClient->addResponse($this->createJsonResponse(200, $responseBody));

        $response = $this->client->sendCustomRequest(new CustomRequest('GET', 'orders/order-123/notes'), CustomNoteResponse::class);

        $this->assertInstanceOf(CustomNoteResponse::class, $response);
        $this->assertSame('hello', $response->note);
    }
     ...
     * @throws JsonException
     */
    public function testSendCustomRequestMapsHttpErrorsLikeBuiltInMethods(): void
    {
        $this->expectException(RequestValidationError::class);

        $this->mockHttpClient->addResponse($this->createJsonResponse(400, json_encode(['message' => 'Invalid note'], JSON_THROW_ON_ERROR)));

        $this->client->sendCustomRequest(new CustomRequest('POST', 'orders/order-123/notes', ['note' => '']));
    }
}
