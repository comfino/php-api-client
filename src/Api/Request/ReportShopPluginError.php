...
{
    /**
     * Payload message format version. Increment on every breaking payload change.
     *
     * Version 1 marks the first structured payload: top-level plugin_version / platform_version / php_version, typed
     * category / severity / context fields, api_endpoint, and occurred_at.
     *
     * The value is transmitted in the Comfino-Message-Version HTTP header rather than inside the body: the header is
     * always readable as plain ASCII before the (compressed, base64-encoded) body is touched, so format detection stays
     * reliable even if the body encoding ever changes.
     */
    public const MESSAGE_VERSION = 1;

    /** Name of the HTTP header conveying {@see MESSAGE_VERSION}. */
    ...
    /**
     * Extracts the canonical API endpoint from a full request URL: host and query string are removed, leaving the path.
     * Used for stable fingerprinting of API-related errors.
     *
     * "https://api.comfino.pl/v1/financial-products?foo=bar" → "v1/financial-products"
     *
     * @param string|null $apiRequestUrl Full API request URL (may be null)
     *
     * @return string|null Canonical endpoint path, or null when no URL was supplied
     */
    public static function extractApiEndpoint(?string $apiRequestUrl): ?string
    {
        ...
        if (!is_string($path) || $path === '') {
            // No path component (e.g., a bare host or a malformed URL): nothing to fingerprint.
            return null;
        }
    ...
    protected function prepareRequestBody(): ?array
    {
        $errorDetailsArray = [
            ...
            'api_request' => $this->shopPluginError->apiRequest !== null
                ? self::truncate(self::sanitizeJsonPayload($this->shopPluginError->apiRequest), self::MAX_API_PAYLOAD_LENGTH)
                : null,
            'api_response' => $this->shopPluginError->apiResponse !== null
                ? self::truncate(self::sanitizeJsonPayload($this->shopPluginError->apiResponse), self::MAX_API_PAYLOAD_LENGTH)
                : null,
            'stack_trace' => $this->shopPluginError->stackTrace !== null
                ? self::filterStackTracePaths(self::truncate($this->shopPluginError->stackTrace, self::MAX_STACK_TRACE_LENGTH))
                : null,
            'occurred_at' => $this->shopPluginError->occurredAt,
        ];

        if (strlen($this->hashKey) < self::MIN_HASH_KEY_LENGTH) {
            throw new RequestValidationError(sprintf('Hash key must be at least %d characters long.', self::MIN_HASH_KEY_LENGTH));
        }
    ...
    /**
     * Removes sensitive keys, coerces values to strings, and limits the size of the environment array before it is
