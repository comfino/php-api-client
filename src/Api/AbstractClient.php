...
abstract class AbstractClient implements ClientInterface
{
    ...
    /** @var string[] */
    private const ALLOWED_API_DOMAINS = ['comfino.pl', 'craty.pl', 'koszulawcraty.pl'];
    ...
    {
        if ($baseUrl !== null && !$this->isAllowedApiBaseUrl($baseUrl)) {
    ...
    protected function sendRequest(Request $request, ?int $apiVersion = null): ResponseInterface
    {
        ...
        if (($trackId = !empty($this->clientHostname) ? $this->clientHostname : gethostname()) === false) {
            $trackId = 'trid-' . bin2hex(random_bytes(8));
        } else {
    ...
    /**
     * Returns true when $url is an acceptable API destination.
     *
     * Rules (in order):
     ...
     * 4. Multi-label hostnames: HTTPS only, and must end with an allowed Comfino domain.
     */
    private function isAllowedApiBaseUrl(string $url): bool
    {
        ...
        // Multi-label hostname: HTTPS only, must be an allowed Comfino domain or its subdomain.
        if ($scheme !== 'https') {
            return false;
        }

        foreach (self::ALLOWED_API_DOMAINS as $domain) {
