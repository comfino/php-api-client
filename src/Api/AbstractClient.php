...
/**
 * Abstract base class for ComfinoPay API clients.
 *
...
abstract class AbstractClient implements ClientInterface
{
     ...
     * @param RequestFactoryInterface $requestFactory The request factory to use for creating API requests (PSR-17 compatible)
     * @param StreamFactoryInterface $streamFactory The stream factory to use for creating request bodies (PSR-17 compatible)
     * @param ?string $apiKey The API key to use for authentication (optional)
     * @param int $apiVersion The API version to use (default: 1)
     * @param ?SerializerInterface $serializer The serializer to use for request and response data (optional, default: JsonSerializer)
     */
    ...
    {
        return $this->customApiBaseUrl ?? ($this->isSandboxMode ? self::SANDBOX_API_BASE_URL : self::PRODUCTION_API_BASE_URL);
    }
     ...
     * - HTTPS URLs on allowed ComfinoPay domains (comfino.pl, craty.pl, koszulawcraty.pl) and their subdomains.
    ...
    public function setCustomApiBaseUrl(?string $baseUrl): void
    {
        if ($baseUrl !== null && !UrlValidator::isAllowedUrl($baseUrl)) {
            throw new InvalidArgumentException(
                "API base URL '$baseUrl' is not allowed. Accepted: ComfinoPay domains (" .
    ...
    /**
     * Returns the version of the ComfinoPay API client.
     *
     ...
     * variable or meta tag) so that browser-side errors logged by the ComfinoPay widget carry the same ID and can be
    ...
    /** @inheritDoc */
    public function createUser(UserRegistration $registration): CreateUserResponse
    {
        return $this->boundClient()->createUser($registration);
    }

    /** @inheritDoc */
    public function fetchAgreements(): FetchAgreementsResponse
    {
        return $this->boundClient()->fetchAgreements();
    }

    /** @inheritDoc */
    public function sendCustomRequest(Request $request, string $responseClass = CustomResponse::class, ?int $apiVersion = null): Response
    {
    ...
    protected function buildContext(): ApiContext
    {
        return new ApiContext(
            ...
            $this->apiCurrency,
            $this->customHeaders,
            $this->customUserAgent ?? "ComfinoPay API client {$this->getVersion()}",
