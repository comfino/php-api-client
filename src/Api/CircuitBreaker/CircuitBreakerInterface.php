 ...
 *
 * When Comfino is unreachable, every call still pays the full timeout times the attempt count before failing. On a
 * single-shop plugin that is one shopper waiting; on a connector fronting hundreds of merchants, it is every worker on
 * the node parked on a dead socket, and a Comfino outage becomes a connector outage. An open breaker turns that wait
 ...
 *  - **Key by tenant as well as host.** One suspended Comfino account must not read as a platform outage.
 */
