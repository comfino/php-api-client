...
use Comfino\Api\Serializer\Factory as SerializerFactory;
use Psr\Http\Message\ResponseInterface;
use Throwable;

/**
 * Abstract base class for ComfinoPay API responses.
 */
abstract class Response
{
    /** @var Request ComfinoPay API client request object associated with this response */
     ...
     *
     * @return Response ComfinoPay API client response object
     *
    ...
    final protected function initFromPsrResponse(): self
    {
        ...
        // Get the content type of the response to determine the serializer to use.
        $contentType = $this->response->hasHeader('Content-Type')
            ? $this->response->getHeader('Content-Type')[0]
            : '';

        $serializerFactory = new SerializerFactory($this->serializer);

        if (!empty($contentType) && $serializerFactory->supports($contentType)) {
            try {
                $deserializedResponseBody = $this->deserializeResponseBody($responseBody, $serializerFactory->createFromContentType($contentType));
        ...
        if ($this->exception === null && $statusCode >= 500) { // @phpstan-ignore identical.alwaysTrue
            throw new ServiceUnavailable(
                "ComfinoPay API service is unavailable: $reasonPhrase [$statusCode]",
                $statusCode,
                null,
