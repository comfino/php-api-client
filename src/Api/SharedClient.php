...
/**
 * Stateless, container-scoped Comfino API client.
 *
...
class SharedClient implements SharedClientInterface
{
    ...
    public function getWidgetKey(ApiContext $context, ?RequestOptions $options = null): string
    {
        ...
        return (new GetWidgetKeyResponse($request, $this->send($context, $request, $options), $this->serializer))
            ->widgetKey;
    }
    ...
    protected function send(
        ...
        if ($this->circuitBreaker->isOpen($breakerKey)) {
            throw new ServiceUnavailable(
                sprintf('Comfino API circuit breaker is open for %s.', $context->getApiHost()),
                503,
                null,
    ...
    private function attempt(
        ...
        if (
            $this->retryExecutor->getRetryPolicy()->classify($retryableResponse, $request->isIdempotent())
            ->isRetryable()
        ) {
    ...
    private function reserveCapacity(ApiContext $context, Request $request, RequestOptions $options): void
    {
        ...
        $message = sprintf(
            'Outbound rate limit exceeded; capacity expected in %d ms.',
    ...
    {
        return $context->userAgent ?? "Comfino API client {$this->getVersion()}";
    }
}
