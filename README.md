<a href="https://developers.comfino.pl">
  <img src="assets/comfinopay_logo.svg" alt="ComfinoPay" width="220">
</a>

# ComfinoPay PHP API client

[![Latest Version](https://img.shields.io/badge/release-3.2.0-blue.svg)](https://github.com/comfino/php-api-client/releases)
...
A portable, PSR-compliant PHP protocol layer for the ComfinoPay payment gateway REST API.
This library handles all HTTP communication with the ComfinoPay API: creating and managing loan applications (orders), querying available financial products, verifying webhook signatures, and generating paywall iframe authentication tokens for the shop checkout page.
...
`createUser()` and `fetchAgreements()` are the one pair of calls made before an API key exists — construct the client
with a `null` key (or, for `SharedClient`, an `ApiContext` whose `apiKey` is `''`):

```php
use Comfino\Api\Dto\Account\UserRegistration;

$client = new Client($httpClient, $requestFactory, $streamFactory, null);

// Fetch the legal agreements to present as consent checkboxes.
$agreements = $client->fetchAgreements()->agreements;

// Register the shop, echoing back the IDs of the agreements the owner accepted.
$registration = new UserRegistration(
    ...
    platformId: 16,
    agreements: array_map(static fn ($agreement) => $agreement->id, array_filter(
        $agreements,
        static fn ($agreement) => $agreement->required
    ))
);

$registered = $client->createUser($registration);

// Store these - every later call authenticates with $registered->apiKey.
$apiKey = $registered->apiKey;
$widgetKey = $registered->widgetKey;
```

A shop already registered under `webSiteUrl` throws `Comfino\Api\Exception\Conflict` (HTTP 409); its message carries
the field-level reason from the API.
...
// Notify ComfinoPay when the payment plugin is uninstalled.
$client->notifyPluginRemoval();

// Notify ComfinoPay of an abandoned cart event.
...
// Pass only $authKey to the frontend widget initialization script served from the ComfinoPay CDN (part of the official ComfinoPay Web Frontend SDK).
```
...
Both are optional and off by default. A breaker stops a ComfinoPay outage from becoming your outage: instead of every
...
the key: without that, `GET /financial-products?loanAmount=130000` and `?loanAmount=130100` are separate buckets, and
the endpoint an integration calls most — with the cart total in the query — is the one the limit never reaches. Scheme,
host and path are kept, so a merchant's sandbox traffic does not spend its production budget.
...
**A shared store needs a compare-and-swap to be exact.** Reserving a token is a read-modify-write, so two workers over
a plain `TokenBucketStoreInterface` both read the same bucket and the second write erases the first — the limiter then
admits one burst *per worker*. Implement `AtomicTokenBucketStoreInterface` (and
`AtomicCircuitBreakerStoreInterface`) instead: the limiter switches to a bounded swap loop, and the breaker uses the
swap to let exactly one worker claim the half-open probe rather than all of them. `TokenBucketRateLimiter::isExact()`
and `CircuitBreaker::isExact()` report which path a given wiring took, so it can be asserted in a test — a shared store
that cannot swap looks identical to one that can, right up to the load that breaks it. `comfino/php-sdk` ships PSR-6
implementations of the plain interfaces (`Psr6TokenBucketStore`, `Psr6CircuitBreakerStore`) for hosts that want shared
state without writing the serialization; PSR-6 itself has no swap, so read their docblocks on which of the two costs
...
try {
    ...
    // ComfinoPay API is temporarily unavailable.
}
```
